...
public void OnPluginStart(){
	...
	hCVMaxGaussAmmo = CreateConVar("sk_max_gauss", "100", "Sets maximum ammo for the gauss weapon.", _, true, 0.0, true, 254.0);
...
public void CG_ItemPostFrame(int client, int weapon){
	...
	if(StrEqual(sWeapon, CLASSNAME)){
		...
		if (GetEntProp(weapon, Prop_Send, "m_iClip1") > hCVMaxGaussAmmo.IntValue)
		{
			setWeaponAmmo(client, hCVMaxGaussAmmo.IntValue, CLASSNAME);
			SetEntProp(client, Prop_Send, "m_iAmmo", hCVMaxGaussAmmo.IntValue, _, GetEntProp(weapon, Prop_Data, "m_iPrimaryAmmoType"));
		}
		SetEntProp(weapon, Prop_Send, "m_iClip1", getClientPrimaryAmmoForWeapon(client, weapon));
		if(m_bCharging[client])
		{
		...
		if ( hitAngle < 0.5 )
		{
			...
			if (entityHit > 0)
			{
 				if(IsPlayer(entityHit)){
					if(!teamplay.BoolValue || GetClientTeam(entityHit) != GetClientTeam(client)){
						float dmgForce[3];
